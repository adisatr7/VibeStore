package com.example.innotechstore.ui.screen.product.laptop
...
@Composable
fun LaptopProductScreen(
    gridHeight: Dp = Dp.Unspecified,
    limit: Int,
    count: Int = 4,
    height: Dp,
    navigateToDetail: (Int) -> Unit,
    viewModel: LaptopProductViewModel = viewModel(
    ...
    viewModel.uiState.observeAsState(initial = UiState.Loading).value.let { uiState ->
        when (uiState) {
            is UiState.Loading -> {
                viewModel.getProductByCategory("laptop", limit)
                Box(
                    modifier = Modifier
