package com.example.innotechstore.ui.screen.product.server
...
@Composable
fun ServerProductScreen(
    gridHeight: Dp = Dp.Unspecified,
    limit: Int,
    height: Dp,
    count: Int = 4,
    navigateToDetail: (Int) -> Unit,
    viewModel: ServerProductViewModel = viewModel(
    ...
    when (uiState) {
        is UiState.Loading -> {
            // Panggil sekali saja saat masuk state Loading
            LaunchedEffect(limit) {
                // ganti kategori ke "electronics" (lebih umum dan sesuai FakeStore-like API)
